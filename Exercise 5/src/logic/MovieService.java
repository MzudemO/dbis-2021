...
public class MovieService extends MovieServiceBase {
	...
	public FindIterable<Document> getBestMovies(int minVotes, double minRating, int limit) {
		FindIterable<Document> result = movies.find(and(gte("votes", minVotes), gte("rating", minRating)))
				.sort(Sorts.descending("rating", "votes")).limit(limit);
		return result;
	}
	...
	public FindIterable<Document> searchByPrefix(String titlePrefix, int limit) {
		Pattern re = Pattern.compile("^" + titlePrefix + ".*");
		FindIterable<Document> result = movies.find(regex("title", re));
		return result;
	}
	...
	public void saveMovieComment(String id, String comment) {
		movies.updateOne(eq("_id", id), Updates.set("comment", comment));
	}
	...
	public FindIterable getByTweetsKeywordRegex(String keyword, int limit) {
		// TODO : implement
		Pattern re = Pattern.compile(".*" + keyword + ".*");
		FindIterable<Document> result = movies.find(elemMatch("tweets", regex("text", re))).limit(limit);
		return result;
	}
	...
	public GridFSFile getFile(String name) {
		...
		if (file == null) {
			file = fs.find(and(eq("metadata.contentType", "image/png"), eq("filename", "sample"))).first();
		}
		return file;
	}
